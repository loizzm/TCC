...
> **ATENÇÃO — relatório parcial.** A sessão rodou com seleção de testes (`paths ['tests/part2/', 'tests/test_part1.py', 'tests/test_leakage.py']`), então nem todos os critérios foram medidos. Regenere com `.venv/bin/python -m pytest -q` sem filtros antes de citar estes números na monografia.
...
| **1.7** | Tempo de geração extrapolado para 6000 amostras | < 15 min (folga 2× sobre os 30 min do PLANO) | 2.71 s para 200 amostras ⇒ 1.36 min | **PASSA** |
...
|---|---|---|---|---|---|
| 200 | 16 | 2.71 | 0.0136 | **1.36** | < 15 min (folga 2× sobre os 30 min do PLANO) |
